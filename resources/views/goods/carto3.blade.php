      ...
      <div class="wrapper1190">
        <div class="layout after-ta order-ha">
          <div class="erhuh">
            <i class="icon-succ"></i>
            <h3>订单提交成功，请您尽快付款！</h3>
            <p class="succ-p">订单号：&nbsp;&nbsp;{{$order->order_sn}}&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp; 付款金额（元）：&nbsp;&nbsp;
              <b>{{$order->total_price}}</b>&nbsp;
              <b>元</b>
            </p>
            @foreach($order->OrderGoods as $key=>$val)
            <div class="succ-tip">&nbsp;&nbsp;
              <b>{{$val->goods_name}}</b>&nbsp;{{$val->spec_key_name}} &nbsp;数量：{{$val->goods_num}} &nbsp;总价：{{$val->goods_num*$val->goods_price}}
            </div>
            @endforeach

          </div>
          <form action="/pay/order/{{$order->id}}" method="post" id="cart4_form">
              {{ csrf_field() }}
            <div class="orde-sjyy">
